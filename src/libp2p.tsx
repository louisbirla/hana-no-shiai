...
import { PrivateKey, keys, randomBytes } from "libp2p-crypto";
...
export const Libp2pAtom = atom<Libp2p>(undefined);
export const PeerIdAtom = atom<PeerId | undefined>(
  (get) => get(Libp2pAtom)?.peerId
);
...
export const Libp2pProvider: React.FC<{ onLoad: () => void }> = ({
  ...
  const [libp2p, setLibp2p] = useAtom(Libp2pAtom);
...
export const getPeerId = async () => {
  ...
  let key = (await keys.generateKeyPairFromSeed("Ed25519", seed, 1024)).bytes;
